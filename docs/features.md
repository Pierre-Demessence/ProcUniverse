# Features

| Feature | Description | Status |
| ------- | ----------- | ------ |
| Pan and zoom camera | Drag to pan; scroll to zoom toward the cursor. | Done (Phase 0) |
| Reference grid | Adaptive world grid and axes for spatial feedback. | Done (Phase 0) |
| FPS HUD | Frame-time / FPS overlay via the engine `stats` module. | Done (Phase 0) |
| Procedural systems | Deterministic star systems generated per sector. | Planned (Phase 1) |
| Analytic orbits | Planets orbit their star (closed-form Kepler). | Planned (Phase 1) |
| LOD streaming | Bounded draw count across every zoom tier. | Planned (Phase 2+) |
| Persistence | World seed plus player deltas only. | Planned (Phase 4) |

Roadmap detail: [plans/procedural-universe.md](plans/procedural-universe.md).
