 ...
 * stays bounded — that is the point of the tier system.
 */
export function renderFrame(deps: FrameDeps): number {
  const { cache, camera, canvas, ctx2d, range, renderer, seed, tier, world } = deps;

  ctx2d.fillStyle = BACKGROUND;
  ctx2d.fillRect(0, 0, canvas.width, canvas.height);

  if (tier === 'galaxy')
    return drawGalaxy(ctx2d, camera, seed);

  if (tier === 'star')
    return drawStars(ctx2d, camera, cache, range);
